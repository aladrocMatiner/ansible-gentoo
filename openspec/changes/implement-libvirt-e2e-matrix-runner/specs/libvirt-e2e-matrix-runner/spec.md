...
The project SHALL provide a Makefile-mediated runner for full disposable libvirt end-to-end validation across the supported amd64 profile/filesystem matrix.

#### Scenario: Run all supported cases
- **WHEN** an operator runs the matrix E2E target with required confirmations
- **THEN** the workflow SHALL run amd64 OpenRC/ext4, amd64 OpenRC/Btrfs, amd64 systemd/ext4, and amd64 systemd/Btrfs
