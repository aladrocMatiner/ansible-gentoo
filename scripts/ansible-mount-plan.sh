...
SCRIPT_NAME=ansible-mount-plan
...
[[ -n "${INSTALL_DISK:-}" ]] || die "INSTALL_DISK is required for mount-plan and has no default"
...
if ! ssh_target_env="$(scripts/vm-ssh-target.sh env)"; then
  die "Unable to discover VM SSH target. Start the Gentoo live VM and verify libvirt DHCP/console networking before running mount-plan."
fi
eval "$ssh_target_env"
