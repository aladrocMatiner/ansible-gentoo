...
load_and_validate_users_inputs() {
  local require_admin_user=${1:-yes}
  ...
  if [[ "$require_admin_user" == yes ]]; then
    [[ -n "$admin_user" ]] || die "ADMIN_USER is required for $SCRIPT_NAME"
  fi
  [[ -z "$admin_user" ]] || validate_username_input ADMIN_USER "$admin_user"
