...
- runs `locale-gen` and `env-update` only when locale files change,
...
The evidence file includes hostname, timezone, locale, keymap, and whether locale generation or env update ran. Later final-check and install-report changes should consume this evidence instead of rediscovering the same state inconsistently.

## Safety

The workflow must not change the live ISO hostname or local controller configuration. All writes are scoped to `/mnt/gentoo`.

If `TIMEZONE`, `LOCALE`, or `KEYMAP` are invalid or missing from the target stage3, the workflow fails before writing partial configuration.

## Recovery

If timezone validation fails, choose a path that exists under target `/usr/share/zoneinfo`.

If keymap validation fails, inspect target `/usr/share/keymaps` and rerun with a supported keymap.

If locale generation fails, inspect `/mnt/gentoo/etc/locale.gen`, target chroot mounts, and `/mnt/gentoo/etc/resolv.conf`, then rerun `make configure-system`.
