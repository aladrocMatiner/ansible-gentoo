...
- VM/libvirt change: update VM docs with ISO path, derived case domain/qcow2/state paths, libvirt URI, network mode, ports or forwarding when implemented, optional `VM_TEST_IMAGE_NAME`, guest `/dev/vda`, case metadata checks, and cleanup behavior.
